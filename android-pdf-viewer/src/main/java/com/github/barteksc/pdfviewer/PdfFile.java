...
public class PdfFile {
    ...
    PdfFile(PdfiumSDK pdfiumSDK, PdfDocument pdfDocument, FitPolicy pageFitPolicy, Size viewSize, int[] originalUserPages,
    ...
    private void setup(Size viewSize) {
        if (originalUserPages != null) {
            ...
            pagesCount = pdfiumSDK.getPageCount(pdfDocument);
        }

        for (int i = 0; i < pagesCount; i++) {
            Size pageSize = pdfiumSDK.getPageSize(pdfDocument, documentPage(i));
    ...
    public void renderPageBitmap(Bitmap bitmap, int pageIndex, Rect bounds, boolean annotationRendering) {
        int docPage = documentPage(pageIndex);
        pdfiumSDK.renderPageBitmap(pdfDocument, bitmap, docPage,
    ...
    public RectF mapRectToDevice(int pageIndex, int startX, int startY, int sizeX, int sizeY, RectF rect) {
        int docPage = documentPage(pageIndex);
        return pdfiumSDK.mapPageCoordinateToDevice(pdfDocument, docPage, startX, startY, sizeX, sizeY, 0, rect);
    }
